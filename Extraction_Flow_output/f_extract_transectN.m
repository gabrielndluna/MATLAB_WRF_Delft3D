function f_extract_transectN(folder,files,startindices,extractid,i,flag)
% extracts data on transect N=i, i.e. a transect line going (under an angle) 
% from the left (west) to the right (east) side of the lake  
...
u       = zeros(tsteps_run,M,K);
v       = zeros(tsteps_run,M,K);
t       = zeros(tsteps_run,M,K);
w       = zeros(tsteps_run,M,K+1);
nuz     = zeros(tsteps_run,M,K+1);
Dz      = zeros(tsteps_run,M,K+1);
tke     = zeros(tsteps_run,M,K+1);
epsilon = zeros(tsteps_run,M,K+1);
